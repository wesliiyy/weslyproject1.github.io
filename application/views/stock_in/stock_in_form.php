<section class="section">
    <div class="section-header">
        <div class="section-header-back">
            <a href="<?= site_url("stock/in") ?>" class="btn btn-primary"><i class="fas fa-arrow-left"></i></a>
        </div>
        <h1>Mutasi Material Masuk</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="<?= site_url("dashboard") ?>">Home</a></div>
            <div class="breadcrumb-item"><a href="<?= site_url("stock/in") ?>">Mutasi In</a></div>
                    ...
                    <div class="card-body col">
                        <form action="<?= site_url("stock/process") ?>" method="post" autocomplete="off">
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-md-6">
                                        <label>Total Berat <small>(Kg)</small>*</label>
                                        <input type="text" name="qty_weight" id="qty_weight" class="form-control">
                                    </div>
                                    <div class="col-md-6">
                                        <label>Total Coil</label>
                                        <input type="text" name="qty_coil" id="qty_coil" class="form-control" readonly>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div>
                                <Button type="submit" name="in_add" class="btn btn-sm btn-primary"><i class="fas fa-save"></i> Save</Button>
...
<script>
    function count_coil() {
        ...
        var qty_weight = $('#qty_weight').val()

        qty_coil = qty_weight / weight_coil
        $('#qty_coil').val(qty_coil.toFixed(1))
    }
    $(document).on('keyup mouseup', '#qty_weight', function() {
        count_coil()
    })
...
<script type="text/javascript">
    $("#qty_weight").keyup(function() {
        var weight_before = parseFloat($("#weight_before").val());
        var qty_weight = parseFloat($("#qty_weight").val());
        var weight_after = weight_before + qty_weight;
        $("#weight_after").val(weight_after);
    });
    $("#qty_weight").keyup(function() {
        var coil_before = parseFloat($("#coil_before").val());
        var qty_weight = parseFloat($("#qty_weight").val());
        var weight_coil = parseFloat($("#weight_coil").val());
        var coil_after = coil_before + qty_weight / weight_coil;
        $("#coil_after").val(coil_after.toFixed(1));
    });
</script>